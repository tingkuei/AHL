...
# RGB
testing_datalist = '/home/tingkuei/IsoGD_dataset/IsoGD_Image/valid_rgb_list.txt'
...
rgb_fc_output = np.zeros((len(y_test),num_classes), dtype=np.float32)
rgb_spp_output = np.zeros((len(y_test), 26880), dtype=np.float32)
load_params = tl.files.load_npz(name='/home/tingkuei/benchmark/IsoGD_pretrain_model/isogr_rgb_model_strategy_3.npz')


np.save('./extracted_feature/valid_y.npy', y_test)
...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
  ...
  rgb_spp_output[test_iterations*batch_size:(test_iterations+1)*batch_size,:]=spp_output
  rgb_fc_output[test_iterations*batch_size:(test_iterations+1)*batch_size,:]=fc_output
...
np.save('./extracted_feature/rgb_valid_spp_output.npy', rgb_spp_output)

# Depth
testing_datalist = '/home/tingkuei/IsoGD_dataset/IsoGD_Image/valid_depth_list.txt'
X_test,y_test = data.load_video_list(testing_datalist)
X_teidx = np.asarray(np.arange(0, len(y_test)), dtype=np.int32)
y_test  = np.asarray(y_test, dtype=np.int32)
depth_spp_output = np.zeros((len(y_test),26880), dtype=np.float32)
...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
  ...
  depth_spp_output[test_iterations*batch_size:(test_iterations+1)*batch_size,:]=spp_output
  depth_fc_output[test_iterations*batch_size:(test_iterations+1)*batch_size,:]=fc_output
  average_accuracy = average_accuracy + accu_value
  test_iterations = test_iterations + 1
average_accuracy = average_accuracy / test_iterations
format_str = ('%s: depth average_accuracy = %.6f')
print (format_str % (datetime.now(), average_accuracy))
np.save('./extracted_feature/depth_valid_spp_output.npy', depth_spp_output)
